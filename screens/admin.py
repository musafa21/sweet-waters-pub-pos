...
class AdminScreen(Screen):
    ...
    def build_ui(self):
        ...
        header.add_widget(Label(text="\u2699\ufe0f  Admin Tools", font_size="18sp", color=WHITE, bold=True))
        main.add_widget(header)

        scroll = ScrollView(bar_width=4, bar_color=(0.3, 0.3, 0.4, 0.3))
        btn_box = BoxLayout(orientation="vertical", size_hint_y=None, spacing=8, padding=[16, 14, 16, 14])
        ...
        actions = [
            ...
            ("\U0001f4dd  Opening Stock", self.opening_stock, (0.2, 0.5, 0.7, 1)),
        ...
        for text, action, color in actions:
            btn = Button(
                ...
                background_color=color, color=WHITE, background_normal="",
                halign="left", padding=[14, 0],
            )
    ...
    def manage_inventory(self, *args):
        ...
        add_label = Label(text="Add new item:", font_size="11sp", color=MUTED, size_hint_y=None, height=20)
        ...
        self._dismiss_popup()
        self._current_popup = Popup(title="Inventory Management", content=content, size_hint=(0.95, 0.88),
        ...
        close_btn = Button(text="Close", size_hint_y=None, height=44,
                           background_color=SURFACE, color=MUTED, background_normal="")
        close_btn.bind(on_release=lambda x: self._dismiss_popup())
    ...
    def _delete_inv_item(self, name):
        ...
        self._populate_inventory_list(self._inv_search.text if hasattr(self, '_inv_search') else "")
    ...
    def manage_staff(self, *args):
        ...
        for user, info in sorted(staff.items()):
            ...
            role_btn = Button(
                ...
                background_color=WARNING if info.get("role") == "admin" else SURFACE, color=WHITE,
            )
            role_u = user
            role_btn.bind(on_release=lambda x, u=role_u: self._toggle_staff_role(u))
            row.add_widget(role_btn)
            pin_btn = Button(text="PIN", font_size="10sp", size_hint_x=0.10,
                             background_color=SURFACE, color=MUTED, background_normal="")
            pin_u = user
            pin_btn.bind(on_release=lambda x, u=pin_u: self._set_staff_pin(u))
            row.add_widget(pin_btn)
            del_btn = Button(text="X", font_size="11sp", size_hint_x=0.10,
                             background_color=DANGER, color=WHITE, background_normal="")
            del_u = user
        ...
        add_label = Label(text="Add new staff:", font_size="11sp", color=MUTED, size_hint_y=None, height=20)
        content.add_widget(add_label)
        add_box = BoxLayout(spacing=6, size_hint_y=None, height=48)
        ...
        content.add_widget(add_box)
        self._staff_error = Label(text="", font_size="11sp", size_hint_y=None, height=18, color=DANGER)
        ...
        self._dismiss_popup()
        self._current_popup = Popup(title="Staff Management", content=content, size_hint=(0.95, 0.85),
                                    auto_dismiss=False, background_color=CARD,
                                    title_color=WHITE, separator_color=DIVIDER)
        close_btn = Button(text="Close", size_hint_y=None, height=44,
                           background_color=SURFACE, color=MUTED, background_normal="")
    ...
    def _toggle_staff_role(self, username):
        ...
        if not acc or username == session.get_user()["username"]:
            return
    ...
    def _set_staff_pin(self, username):
        from backend.database import load_json, save_json, encrypt_pin
        self._haptic()
        content = BoxLayout(orientation="vertical", spacing=10, padding=16)
        content.add_widget(Label(text=f"Set PIN for {username}", font_size="14sp", color=WHITE))
        pin_input = TextInput(
        ...
        err_label = Label(text="", font_size="11sp", color=DANGER, size_hint_y=None, height=18)
    ...
    def opening_stock(self, *args):
        ...
        existing = movements.get("opening", {}).get(dk, {})

        content = BoxLayout(orientation="vertical", spacing=4, padding=12)
        content.add_widget(Label(text=f"Set opening stock for {dk}", size_hint_y=None, height=28, color=MUTED))

        self._opening_search = TextInput(
        ...
        self._opening_search.bind(text=self._filter_opening)
        content.add_widget(self._opening_search)

        self._opening_fields = {}
        self._opening_existing = existing
        scroll = ScrollView()
        self._opening_box = BoxLayout(orientation="vertical", size_hint_y=None, spacing=2)
        self._opening_box.bind(minimum_height=self._opening_box.setter("height"))
        self._populate_opening()
        scroll.add_widget(self._opening_box)
        content.add_widget(scroll)

        popup = Popup(title="Set Opening Stock", content=content, size_hint=(0.95, 0.88),
                      auto_dismiss=False, background_color=CARD,
                      title_color=WHITE, separator_color=DIVIDER)
        save_btn = Button(text="Save", size_hint_y=None, height=44,
                          background_color=SUCCESS, color=WHITE, background_normal="")
        save_btn.bind(on_release=lambda x: (self._save_opening(dk), popup.dismiss()))
    ...
    def _populate_opening(self, filter_text=""):
        from backend.stock import get_stock_list
        self._opening_box.clear_widgets()
        self._opening_box.height = 0
        count = 0
        for name in sorted(get_stock_list().keys()):
            if filter_text and filter_text.lower() not in name.lower():
                continue
            existing_val = self._opening_existing.get(name, "")
            row = BoxLayout(size_hint_y=None, height=40, spacing=4)
            row.add_widget(Label(text=name, font_size="11sp", halign="left",
                                 text_size=(None, None), size_hint_x=0.55, color=WHITE))
            field = TextInput(
                text=str(existing_val) if existing_val != "" else "",
                hint_text="0", input_filter="int",
                font_size="13sp", multiline=False, size_hint_x=0.3,
                background_color=INPUT_BG, background_normal="",
                foreground_color=WHITE, cursor_color=ACCENT,
                hint_text_color=HINT, padding=[8, 8],
            )
            self._opening_fields[name] = field
            row.add_widget(field)
            self._opening_box.add_widget(row)
            count += 1
        self._opening_box.height = count * 42

    def _filter_opening(self, instance, text):
        saved = {}
        for name, field in self._opening_fields.items():
            if field.text:
                saved[name] = field.text
        self._populate_opening(text)
        for name, field in self._opening_fields.items():
            if name in saved:
                field.text = saved[name]

    def _save_opening(self, dk):
        ...
        for name, field in self._opening_fields.items():
            try:
                ...
                pass
        movements = load_stock_movements()
        movements.setdefault("opening", {})[dk] = opening
        save_stock_movements(movements)
        self._log_audit(f"Saved opening stock for {dk}")
    ...
    def add_stock(self, *args):
        ...
        dk = today_key()
        content = BoxLayout(orientation="vertical", spacing=6, padding=12)
        content.add_widget(Label(text=f"Date: {dk}", size_hint_y=None, height=32, color=MUTED))
        ...
        self._dismiss_popup()
        self._current_popup = Popup(title="Add Stock", content=content, size_hint=(0.95, 0.8),
                                    auto_dismiss=False, background_color=CARD,
                                    title_color=WHITE, separator_color=DIVIDER)
        save_btn = Button(text="Save All", size_hint_y=None, height=44,
                          background_color=SUCCESS, color=WHITE, background_normal="")
        save_btn.bind(on_release=lambda x: (self._save_stock_entries(dk), self._dismiss_popup()))
        self._current_popup.bind(on_dismiss=lambda: self._warn_unsaved_stock())
    ...
    def stock_take(self, *args):
        ...
        content = BoxLayout(orientation="vertical", spacing=4, padding=12)
        content.add_widget(Label(text="Enter closing stock counts", size_hint_y=None, height=28, color=MUTED))
        ...
        self._dismiss_popup()
        self._current_popup = Popup(title=f"Stock Taking - {dk}", content=content, size_hint=(0.95, 0.88),
                                    auto_dismiss=False, background_color=CARD,
                                    title_color=WHITE, separator_color=DIVIDER)
        save_btn = Button(text="Save", size_hint_y=None, height=44,
                          background_color=SUCCESS, color=WHITE, background_normal="")
        save_btn.bind(on_release=lambda x: (self._save_stock_take(dk), self._dismiss_popup()))
        cancel_btn = Button(text="Cancel", size_hint_y=None, height=44,
                            background_color=SURFACE, color=MUTED, background_normal="")
        cancel_btn.bind(on_release=self._dismiss_popup)
        btn_box = BoxLayout(spacing=6, size_hint_y=None, height=48)
        btn_box.add_widget(cancel_btn)
        btn_box.add_widget(save_btn)
        content.add_widget(btn_box)
    ...
    def edit_prices(self, *args):
        ...
        content = BoxLayout(orientation="vertical", spacing=4, padding=12)
        content.add_widget(Label(text="Enter new prices", size_hint_y=None, height=28, color=MUTED))

        self._price_search = TextInput(
            hint_text="  Search items...", size_hint_y=None, height=38,
            multiline=False, font_size="13sp",
            background_color=INPUT_BG, background_normal="",
            foreground_color=WHITE, cursor_color=ACCENT,
        ...
        self._price_search.bind(text=self._filter_prices)
        content.add_widget(self._price_search)

        self._price_fields = {}
        self._price_data = get_stock_list()
        scroll = ScrollView()
        self._price_box = BoxLayout(orientation="vertical", size_hint_y=None, spacing=2)
        self._price_box.bind(minimum_height=self._price_box.setter("height"))
        self._populate_prices()
        scroll.add_widget(self._price_box)
        content.add_widget(scroll)

        self._dismiss_popup()
        self._current_popup = Popup(title="Edit Prices", content=content, size_hint=(0.95, 0.88),
                                    auto_dismiss=False, background_color=CARD,
                                    title_color=WHITE, separator_color=DIVIDER)
        save_btn = Button(text="Save", size_hint_y=None, height=44,
                          background_color=SUCCESS, color=WHITE, background_normal="")
        save_btn.bind(on_release=lambda x: (self._save_prices(), self._dismiss_popup()))
        cancel_btn = Button(text="Cancel", size_hint_y=None, height=44,
                            background_color=SURFACE, color=MUTED, background_normal="")
        cancel_btn.bind(on_release=self._dismiss_popup)
        btn_box = BoxLayout(spacing=6, size_hint_y=None, height=48)
    ...
    def _populate_prices(self, filter_text=""):
        self._price_box.clear_widgets()
        self._price_box.height = 0
        count = 0
        for name, info in sorted(self._price_data.items()):
            if filter_text and filter_text.lower() not in name.lower():
                continue
            row = BoxLayout(size_hint_y=None, height=40, spacing=4)
            row.add_widget(Label(text=name, font_size="11sp", halign="left",
            ...
            row.add_widget(field)
            self._price_box.add_widget(row)
            count += 1
        self._price_box.height = count * 42

    def _filter_prices(self, instance, text):
        if hasattr(self, '_price_fields'):
            saved = {}
            for name, field in self._price_fields.items():
                if field.text:
                    saved[name] = field.text
            self._populate_prices(text)
            for name, field in self._price_fields.items():
                if name in saved:
                    field.text = saved[name]
        else:
    ...
    def export_backup(self, *args):
        ...
        try:
            ...
            content.add_widget(Label(
                text=f"Backup saved!\n\n{backup_path}",
                font_size="13sp", halign="center", color=WHITE, text_size=(280, None),
            ))
            ...
            content.add_widget(Label(
                ...
                font_size="13sp", halign="center", color=DANGER, text_size=(280, None),
            ))
            ok_btn = Button(text="OK", size_hint_y=None, height=44,
                            background_color=SURFACE, color=MUTED, background_normal="")
            content.add_widget(ok_btn)
    ...
    def reset_data(self, *args):
        ...
        content.add_widget(Label(
            text="Enter admin password to confirm:", font_size="13sp", halign="center", color=MUTED,
        ))
        ...
        self._reset_error = Label(text="", font_size="11sp", size_hint_y=None, height=20, color=DANGER)
        ...
        reset_btn = Button(text="RESET ALL", background_color=DANGER, color=WHITE, background_normal="")
        btn_box.add_widget(cancel_btn)
